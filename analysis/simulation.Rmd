...
################### Binary Outcomes #########################
############## Target FDR = 0.2 ###############
bin_rf <- suppressWarnings(zinck.filter(X,X_tilde,as.factor(Y2),model="Random Forest",fdr=0.2,offset=0,mtry=14,seed=15,rftuning = TRUE))
...
set.seed(3)
imp <- stat.random_forest(Xlog,Xlog_tilde,Y1)
T <- knockoff.threshold(imp,fdr=0.2)
...
paste("MX-KF (continuous) metrics:")
print(paste("Estimated FDR for Target 0.2:", estimated_FDR_KF_cts))
print(paste("Estimated Power for Target 0.2:", estimated_power_KF_cts))

############### Binary Outcomes (MX-KF) ###################
set.seed(21)
imp <- stat.random_forest(Xlog,Xlog_tilde,as.factor(Y2))
T <- knockoff.threshold(imp,fdr=0.2)
index_est <- which(imp >=T)
...
paste("MX-KF (Binary) metrics:")
print(paste("Estimated FDR for Target 0.2:", estimated_FDR_KF_bin))
print(paste("Estimated Power for Target 0.2:", estimated_power_KF_bin))
...
set.seed(34)
imp <- stat.random_forest(X,X_tilde.LDA,Y1)
T <- knockoff.threshold(imp,fdr=0.2,offset=0)
index_est <- which(imp>=T)
...
paste("LDA-KF (continuous) metrics:")
print(paste("Estimated FDR for Target 0.2:", estimated_FDR_LDA_cts))
print(paste("Estimated Power for Target 0.2:", estimated_power_LDA_cts))

############### Binary Outcomes (LDA-KF) ###################

set.seed(34)
imp <- stat.random_forest(X,X_tilde.LDA,as.factor(Y2))
T <- knockoff.threshold(imp,fdr=0.2,offset=0)
index_est <- which(imp >=T)
...
paste("LDA-KF (Binary) metrics:")
print(paste("Estimated FDR for Target 0.2:", estimated_FDR_LDA_bin))
print(paste("Estimated Power for Target 0.2:", estimated_power_LDA_bin))
